...
// TestPEMinimumTargetVersion checks that cmd/link stamps the 5.1
	...
	for _, arch := range []string{"386", "amd64"} {
		t.Run(arch, func(t *testing.T) {
			...
			if osMajor != 5 || osMinor != 1 {
				t.Errorf("operating system version is %d.%d, want 5.1", osMajor, osMinor)
			}
			if subMajor != 5 || subMinor != 1 {
				t.Errorf("subsystem version is %d.%d, want 5.1", subMajor, subMinor)
			}
		})
	}
}
