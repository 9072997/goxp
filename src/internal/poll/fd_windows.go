...
func (fd *FD) waitEventIO(o *operation) error {
	...
	for {
		...
		case s == syscall.WAIT_OBJECT_0+1:
			if fd.closing() && !windows.SupportCancelIoEx() {
				// Close signalled this wait. Without CancelIoEx its cancel
				// cannot reach an operation issued by another thread, so
				// the cancel has to come from here, the thread execIO keeps
				// this operation on.
				return fd.cancelEventIO(o, errClosing(fd.isFile))
			}
...
func (fd *FD) Fchmod(mode uint32) error {
	...
	// Not SetFileInformationByHandle directly: that is Vista and later, and
	// this wrapper falls back to the native call it wraps where it is absent.
	return windows.SetFileBasicInfoByHandle(fd.Sysfd, &du)
}
