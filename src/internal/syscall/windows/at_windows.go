...
func ObjDontReparseUnsupportedForTest() bool {
	return !objDontReparseSupported()
}
...
func Openat(dirfd syscall.Handle, name string, flag uint64, perm uint32) (_ syscall.Handle, e1 error) {
	...
	emulateNoFollow := noFollow && (TestOpenatNoObjDontReparse || !objDontReparseSupported())
	...
	if emulateNoFollow {
		...
		// to traverse it, then refuse the handle below if that is what we got.
	...
	if emulateNoFollow && fileFlags&O_FILE_FLAG_OPEN_REPARSE_POINT == 0 {
		// The emulated OBJ_DONT_REPARSE refusal, on a handle already held, so
		// nothing can be swapped underneath it between the test and the use.
		// It comes before O_TRUNC below, which would otherwise truncate a link
		// that is about to be refused. A caller that asked for
		// O_FILE_FLAG_OPEN_REPARSE_POINT (Lstat, Readlink) wants the link
		// itself, which is also what OBJ_DONT_REPARSE gives it.
...
func deleteatFallback(h syscall.Handle, objAttrs *OBJECT_ATTRIBUTES, name string) error {
	...
	err := setFileDispositionByHandle(h)
	if err != nil && aside {
...
//
// ReOpenFile cannot do this. It is not in XP SP3's kernel32, and it refuses
// directories: measured on Windows 11, it failed with ERROR_ACCESS_DENIED for
// every access mask and share mode tried, where this NtOpenFile succeeded.
